...
function main() {
    fs.readFile('sample.txt', (err, data) => {
        ...
        const buckets = initializeBuckets(pipelines, items);

        moveItemsBetweenBuckets(buckets);

        console.log(calculateSum(buckets));
    });
}
...
function parseRule(rawRule) {
    if (!rawRule.includes(':')) {
        return { passes: () => true, target: rawRule };
    }
...
function initializeBuckets(pipelines, items) {
...
function moveItemsBetweenBuckets(buckets) {
    let moved = true;
...
function calculateSum(buckets) {
    return [...buckets.get('A').items].reduce((sum, item) => {
