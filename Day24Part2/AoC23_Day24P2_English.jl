...
# ╔═╡ 1b55aa42-4a3a-40ee-b2ae-34ae87276038
md"![](https://raw.githubusercontent.com/orfeasa/advent-of-code-2023/master/header.png)"
...
# ╔═╡ 823e7ba0-985a-4528-9f60-b9d6a6679028
begin
	p = [19, 13, 30]
	v_p = [-2, 1, -2]
	...
	plot(
		x, y, z,
		title = "Trajectory",
		label = L"f(t) = \vec{P_0} + \vec{v} \cdot t",
		size = (600,380),
		linewidth = 3,
		camera = (20, 25)
	)
...
# ╔═╡ 0ceb33bf-edbe-4953-879f-6fa5c7eb436f
md"""
## Collision

Two bodies, $P$ and $Q$, collide when, at a given time $t$, their positions are equal:

$$\vec{P_0} + \vec{v_P} \cdot t = \vec{Q_0} + \vec{v_Q} \cdot t$$
"""
...
Let $\vec{P_0} = \begin{pmatrix} 19 & 13 & 30 \end{pmatrix}$ and $\vec{v_P} = \begin{pmatrix} -2 & 1 & -2 \end{pmatrix}$.

Let $\vec{Q_0} = \begin{pmatrix} 24 & 13 & 10 \end{pmatrix}$ and $\vec{v_Q} = \begin{pmatrix} -3 & 1 & 2 \end{pmatrix}$.

To find the collision point, we can plot the trajectories $f(t) = \vec{P_0} + \vec{v_P} \cdot t$ and $g(t) = \vec{Q_0} + \vec{v_Q} \cdot t$. The intersection point of these two lines represents the collision:
"""
...
begin
	...
	plot!(
		xq, yq, zq,
		title = "Trajectory",
		label = L"g(t) = \vec{Q_0} + \vec{v_Q} \cdot t",
		size = (600,380),
		linewidth = 3,
	)
	scatter3d!(
